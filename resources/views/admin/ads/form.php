...
<form method="POST" action="<?= $ad ? '/admin/ads/' . (int) $ad['id'] : '/admin/ads' ?>" enctype="multipart/form-data" class="glass-card" style="padding:24px;max-width:640px;">
  ...
  <div class="field">
    <label>Ad type</label>
    <select class="input" name="type" id="ad_type" onchange="document.getElementById('destination_field').style.display = this.value === 'external' ? '' : 'none';">
    ...
    <div class="field-hint">Image ads show the banner while the user watches. External ads also open the destination URL in a new tab.</div>
  </div>

  <div class="field"><label>Banner image<?= $adType === 'external' ? ' (optional)' : '' ?></label><input class="input" type="file" name="image" accept="image/*">
  ...
  <div id="destination_field" class="field" style="<?= $adType === 'external' ? '' : 'display:none;' ?>">
    <label>Destination URL</label>
    <input class="input" type="url" name="destination_url" value="<?= e($ad['destination_url'] ?? old('destination_url')) ?>">
  </div>
